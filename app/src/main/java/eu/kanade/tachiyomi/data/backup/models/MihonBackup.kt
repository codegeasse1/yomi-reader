...
data class MihonBackup(
    ...
    /**
     * Yomi's own compatibility manifest.
     *
     ...
     */
    @ProtoNumber(YomiSisterManifest.PROTO_FIELD) var tadamiManifest: YomiSisterManifest? = null,
) {
    /**
     ...
     */
    fun validManifestHints(): Map<Pair<Long, String>, YomiMediaType>? {
        return tadamiManifest?.takeIf { it.isValid }?.hintsByKey()
    }

    fun toYomiBackup(): Backup {
        return Backup(
...
/**
 * Flatten a Yomi backup into the Mihon shape so other apps can read it.
 *
...
internal fun Backup.toMihonBackup(): MihonBackup {
    val hints = backupManga.map {
        YomiMediaTypeHint(sourceId = it.source, url = it.url, mediaType = YomiMediaType.MANGA)
    } + backupNovel.map {
        YomiMediaTypeHint(sourceId = it.source, url = it.url, mediaType = YomiMediaType.NOVEL)
    }
    ...
    return MihonBackup(
        ...
        tadamiManifest = YomiSisterManifest(entries = hints),
    )
}
 ...
 * `memo` (ByteArray), whereas Yomi uses 13 for `dateUploadRaw` (String). Both are
 ...
 *
 * Mihon and Yomi/Aniyomi share field numbers 1..109 but DIVERGE afterwards.
 * Decoding a Mihon backup with Yomi's [BackupManga] throws a wire-type mismatch
 * (Yomi expects `rating: Float` (fixed32) at 110 and `notes: String` at 111,
 * while Mihon stores `notes: String` at 110 and `initialized: Boolean` at 111).
 *
 * Real Mihon layout (verified against mihonapp/mihon):
 *   108 -> excludedScanlators (same as Yomi)
 *   109 -> version            (same as Yomi)
 *   110 -> notes              (Yomi: rating)
 *   111 -> initialized        (Yomi: notes)      -- omitted, skipped on decode
 *   112 -> memo               (Yomi: none)       -- omitted, skipped on decode
 */
@Serializable
data class MihonBackupManga(
    ...
    // Yomi has no equivalent; the decoder skips them.
) {
