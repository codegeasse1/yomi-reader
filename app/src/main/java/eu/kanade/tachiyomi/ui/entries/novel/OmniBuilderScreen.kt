        ...
        LaunchedEffect(state) {
            ...
            if (current is OmniBuilderScreenModel.State.Active) {
                webViewRef?.evaluateJavascript(
                    "window.YomiPicker.setInteractionMode(${current.isInteractionMode});",
                    null,
                )
            }
        }
            ...
            AndroidView(
                ...
                factory = { context ->
                    WebView(context).apply {
                        ...
                        addJavascriptInterface(screenModel.bridge, "YomiBridge")

                        webViewClient = object : WebViewClient() {
                            override fun onPageFinished(view: WebView, url: String) {
                                super.onPageFinished(view, url)
                                val js = try {
                                    context.assets.open("YomiPicker.js").bufferedReader().use { it.readText() }
                                } catch (e: Exception) {
                                    null
                                }
                                if (js != null) {
                                    view.evaluateJavascript(js, null)
                                    view.evaluateJavascript("window.YomiPicker.enable();", null)
                                }
