...
/**
 * Pure planning layer for the Shikimori importer.
 * Mirrors [tachiyomi.data.anixart.AnixartImportPlanner] for dedup and category mapping.
 */
object ShikimoriImportPlanner {
    ...
    data class Config(
        val statusCategoryIds: Map<ShikimoriImportStatus, Long> = emptyMap(),
    )
    ...
    fun plan(selections: List<Selection>, config: Config): Plan {
        ...
        for (sel in selections) {
            ...
            val status = ShikimoriImportStatus.fromApi(sel.entry.status)
            val cats = buildSet {
